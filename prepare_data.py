...
def get_frequency_dataloader(label_smoothing: bool = True, batch_size: int = 64):
    ...
    Training set is augmented by frequency filtering.
    Validation and test set are un-augmented raw data.
    """
    prepare_frequency_data()

    X_train_freq = np.load("data/generated/frequency/X_train_valid_augmented.npy")
    y_train_freq = np.load("data/generated/frequency/y_train_valid_augmented.npy")
    ...
    split_idx = 1777
    X_train_data, X_valid_data = X_train_freq[:split_idx], X_train_raw[split_idx:]
    y_train_data, y_valid_data = y_train_freq[:split_idx], y_train_raw[split_idx:]
